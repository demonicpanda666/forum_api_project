...
describe('/threads endpoint', () => {
  ...
  describe('when POST /threads', () => {
    ...
    it('should response 201 and create new thread', async () => {
      ...
      expect(responseJson.data.createdThread.title).toEqual('sebuah thread');
    });
  });
  describe('when GET /threads/{threadId}', () => {
    ...
    it('should response 200 and return detail thread', async () => {
      ...
      const response = await server.inject({
        method: 'GET',
        url: `/threads/${threadResponse.data.createdThread.id}`,
        headers: { Authorization: `Bearer ${responseAuth.data.accessToken}` },
      });
      ...
      expect(responseJson.status).toEqual('success');
      expect(responseJson.data.thread.id).toEqual(threadResponse.data.createdThread.id);
